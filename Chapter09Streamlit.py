...
def Chapter9StreamlitUI():
   ...
   # Giao diện Streamlit
    st.title("Ứng dụng Xử lý Ảnh Nhị Phân", anchor="binary_app-title")
    ...
    with st.container():
        with st.expander("Cài đặt Xử lý Ảnh", expanded=True):
            selected_option = st.selectbox(
                "Chọn kỹ thuật xử lý",
                processing_options,
                key="binary_processing_selectbox"
            )
            if selected_option:
            ...
            use_sample_image = st.checkbox(
                "Sử dụng ảnh mẫu",
                value=False,
                key="binary_sample_image_checkbox"
            )
            ...
            if not use_sample_image:
                image_file = st.file_uploader(
                    "Tải ảnh lên",
                    type=["png", "jpg", "jpeg", "bmp", "tif"],
                    key="binary_main_uploader"
                )
        
        st.markdown('<div id="binary_settings-container"></div>', unsafe_allow_html=True)
